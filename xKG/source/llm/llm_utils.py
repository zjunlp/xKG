...
KNOWN_LANGUAGES = {
    'python', 'py', 'json', 'javascript', 'js', 'html', 'css', 
    'bash', 'sh', 'shell', 'sql', 'java', 'c', 'cpp', 'c++', 
    'csharp', 'cs', 'go', 'rust', 'typescript', 'ts', 'yaml', 
    'yml', 'markdown', 'md', 'xml', 'text', 'txt'
}
...
def extract_backtick_texts(text: str) -> list[str]:
    raw_contents = re.findall(r'```([\s\S]*?)```', text)
    clean_results = []
    for raw in raw_contents:
        parts = raw.split('\n', 1)
        if len(parts) > 1 and parts[0].strip().lower() in KNOWN_LANGUAGES:
            content = parts[1]
        else:
            content = raw
        clean_results.append(content.strip())
