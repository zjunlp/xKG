...
Now please think and reason carefully, and wrap your final answer between two ``` in the end.
    ...
    def run(
        ...
        if extracted_links_with_context:
            formatted_snippets = []
            for before, url, after in extracted_links_with_context:
                snippet = f"...{before.strip()} {url} {after.strip()}..."
            ...
            code_prompt = self._prompt_get_github_url.format(
                ...
                github_link=github_link_context
            )
            ...
            response_text = extract_object(extract_backtick_text(response[0]))

            github_url_check_pattern = re.compile(r'https?://github\.com/[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+/?$')
            try:
                if response_text and github_url_check_pattern.match(response_text.strip()):
                    paper.code_url = response_text.strip()
                else:
                    paper.code_url = None
            except Exception as e:
                logger.warning(f"Failed to validate GitHub URL from LLM response: {response_text} with error: {e}")
                paper.code_url = None
